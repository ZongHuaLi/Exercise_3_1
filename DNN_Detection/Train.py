...
def train(config):    
    ...
    if os.path.isfile(Pilot_file_name):
        ...
        print (f'Creating new Pilot file: {Pilot_file_name}') 
        bits_pilot = np.random.binomial(n=1, p=0.5, size=(K*mu, ))
        np.savetxt(Pilot_file_name, bits_pilot, delimiter=',')
    ...
    with tf.Session(config=config_GPU) as sess:
        ...
        for epoch in range(traing_epochs):
            ...
            if epoch % model_saving_step == 0:
                # 取得儲存目錄的路徑
                save_dir = config.Model_path + 'SNR_' + str(SNRdb) + '/'
                # 如果目錄不存在，自動建立
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                
                saving_name = save_dir + 'DetectionModel_SNR_' + str(SNRdb) + '_Pilot_' + str(P) + '_epoch_' + str(epoch)
