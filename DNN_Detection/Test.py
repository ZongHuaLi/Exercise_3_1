...
def test(config): # 加入 config参数
        ...
        with tf.Session(config=tf_config) as sess: # 修正錯誤
            ...
            #print("OFDM Detection QAM output number is", n_output, "SNR = ", config.SNRdb, "Num Pilot", config.P,"prediction and the mean error on test set are:", mean_error.eval({X:batch_x}), BER.eval({X:batch_x}))
            #修正 Print 裡面的變數名稱
